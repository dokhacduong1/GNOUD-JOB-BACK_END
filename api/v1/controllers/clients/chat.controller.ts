...
// [POST] /api/v1/client/chat/get-content-chat
export const getContentChat = async function (
  ...
  try {
    ...
    const roomExist = await RoomChat.findOne({_id: req["roomChat"]})
...
// [GET] /api/v1/client/chat/get-history-chat
...
export const getHistoryChat = async function (
  ...
  try {
    ...
    const populateInfo: POPULATE[] = [
      {
        path: "users.employer_id", // Đường dẫn đến trường cần populate
        select: "email phone fullName logoCompany image", // Chọn các trường cần lấy từ User
        model: Employer, // Model cần populate
      },
    ];
    ...
    const data = rooms.map((room) => {
      ...
      const user = room.users[findIndexEmployer]?.employer_id || {};
      return {
        typeRoom: "friend",
        room_chat_id: room._id, // id phòng chat
        user_id: user["_id"], // id người dùng
        fullName: user["fullName"], // tên đầy đủ người dùng
        avatar: user["image"], // avatar người dùng
