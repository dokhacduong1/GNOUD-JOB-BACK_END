...
export async function detailedAddress(ward: string, district: string, city: string, input: string): Promise<any> {
  try {
    ...
    const url : string = process.env.AUTOCOMPLETE+district_C+component_C+ward_C+input_C+sessiontoken_C+state_C+use_case_C+v_C
...
export async function Coordinate(placeid: string): Promise<any> {
    try {
        ...
        const url :string = process.env.DETAILS_LOCATION+component_C+fields_C+group_C+placeid_C+sessiontoken_C+use_case_C+v_C
